...
function wmpStop(){
 ...
 Exobud.close();
 disp1.innerHTML="CnSide MP 播放已停止";
 if(blnStatusBar){window.status=('CnSide MP');return true;}
}
...
function evtWmpError(){
 ...
 if(intErrCnt<=3){
	disp2.innerHTML="CnSide MP("+intErrCnt+")";
	var msg="(CnSide MP:" + intErrCnt +") " +cActTit;
	disp1.innerHTML="CnSide MP" +cActTit;
	if(blnStatusBar){window.status=(msg);}
...
function waitMsg(){
 capText.innerHTML="";
 if(intExobudStat ==1){disp1.innerHTML="CnSide MP Player";}
 if(intExobudStat ==2){disp1.innerHTML="CnSide MP Player";}
 if(blnStatusBar){
	if(intExobudStat ==1){window.status=('CnSide MP');return true;}
	if(intExobudStat ==2){window.status=('CnSide MP'); return true;}
 }
}


function openPlist() { 
	window.open("cnsidepl.html" , "mplist", " width=280 , height=498 ,scrollbars=auto , resizable=no , copyhistory=no");
}
...
function vizExobud(){
//Exobud.launchURL("http://www.cnside.com");
  window.open("http://www.cnside.com","vizExobud");
}
